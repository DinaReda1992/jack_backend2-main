...
@section('js_files')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('input[name="user_type_id"]').click(function () {
                var type = $(this).val();
                if (type == 3) {
                    $('.prevs').hide();
                } else {
        ...
        <div class="page-header">
            <div class="page-header-content">
                <div class="page-title">
                    <h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">لوحة التحكم </span>
                        - {{ isset($object)? 'تعديل':'إضافة' }} مجموعة</h4>
                </div>
                <div class="heading-elements">
                    <div class="heading-btn-group">
                        <a href="/provider-panel/groups">
                            <button type="button" class="btn btn-primary" name="button"> عرض مجموعات الصلاحيات</button>
                        </a>
                    </div>
                </div>
                <!-- 						<div class="heading-elements"> -->
            ...
            <div class="breadcrumb-line">
                <ul class="breadcrumb">
                    ...
                    <li><a href="/provider-panel/groups">عرض مجموعات الصلاحيات</a></li>
                    <li class="active">{{ isset($object)? 'تعديل':'إضافة' }} مجموعة</li>
                </ul>
        ...
        <!-- Form horizontal -->
            <div class="panel panel-flat">
                <div class="panel-heading">
                    <h5 class="panel-title">{{ isset($object)? 'تعديل':'إضافة' }} مجموعة </h5>
                    <div class="heading-elements">
                        <ul class="icons-list">
                ...
                <div class="panel-body">
                    <form method="post" class="form-horizontal"
                          action="{{ isset($object)? '/provider-panel/groups/'.$object->id : '/provider-panel/groups'  }}">
                        {!! csrf_field() !!}
                        @if(isset($object))
                        ...
                        <fieldset class="content-group">
                            ...
                            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                <label class="control-label col-lg-2">أدخل اسم المجموعة</label>
                                <div class="col-lg-10">
                                    <input type="text" name="name"
                                           value="{{ isset($object) ? $object->name  : old('name')  }}"
                                           class="form-control" placeholder="أدخل اسم المجموعة">
                                    @if ($errors->has('name'))
                            ...
                            <div class="form-group prevs">
                                <label class="control-label col-lg-2">الصلاحيات</label>
                                    ...
                                    @foreach($privileges as $privilege)
                                        ...
                                        <section>
                                            <h3><label><input type="checkbox" name="privileges[]"
                                                              value="{{ $privilege->id }}" <?php if (in_array($privilege->id, $prevs)) {
                                                        echo "checked";
                                                    } ?> />
                                                    {{ $privilege->privilge }}
                                                </label></h3>
                        ...
                        <div class="text-right">
                            <button type="submit" class="btn btn-primary">{{ isset($object)? 'تعديل':'إضافة' }} المجموعة

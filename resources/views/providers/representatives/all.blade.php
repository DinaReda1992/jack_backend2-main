				...
				<table class="table table-bordered">
					...
					@foreach($objects as $object)
						<tr parent_id="{{ $object->id }}">
							...
							<td><a target="_blank" href="/provider-panel/all-users/{{ $object->user_id }}/edit">{{ @$object->getUser->username }}</a></td>
							<td>{{ @$object->full_name }} </td>
							<td>
							<a href="/provider-panel/approve-request/{{ $object->id }}">
							<button type="button" name="button" class="btn btn-success"> موافقة على الطلب</button>
							</a>
							</td>
							<td>
							<a onclick="return false;" data-toggle="modal"
							data-target="#myModal{{ $object->id }}555" href="#">
							<button type="button" name="button" class="btn btn-danger"> رفض الطلب </button>
							</a>
							...
							<h4 class="modal-title" id="myModalLabel">رسالة رفض الطلب</h4>
							</div>
							<form method="get" action="/provider-panel/cancel_request/{{ $object ->id }}">
							<div class="modal-body">
							<div class="form-group">
							<label for="exampleInputEmail1"></label>
							<textarea name="reason_of_cancel" class="form-control"></textarea>
							</div>
									...
									<div class="modal-dialog" role="document">
										<div class="modal-content">
											...
											<div class="modal-body">
												<hr>
												<div class="row">
													<label class="col-sm-4"><strong> الاسم الثلاثي </strong></label>
													<div class="col-sm-8">{{ @$object->full_name }}</div>
												</div>
												...
												@if($object->car_plate)
													<div class="row">
														<label class="col-sm-4"><strong> رقم السيارة</strong></label>
														<div class="col-sm-8">{{ @$object->car_plate }}</div>
													</div>
													<hr>
												@endif
												@if($object->bank_id)
													<div class="row">
														<label class="col-sm-4"><strong> الينك التابع له المندوب</strong></label>
														<div class="col-sm-8">{{ @$object->bank->name }}</div>
													</div>
													<hr>
												@endif
												@if($object->bank_account)
													<div class="row">
														...
														<div class="col-sm-8">{{ @$object->bank_account }}</div>
													</div>
													<hr>
												@endif
{{--												@if($object->photo)--}}
...
{{--												@endif--}}
												@if($object->liscense)
													<div class="row">
														<label class="col-sm-4"><strong> صورة استمارة السيارة</strong></label>
...
{{--												@endif--}}
							...
							<li class="text-danger-600"><a onclick="return false;" object_id="{{ $object->id }}" delete_url="/provider-panel/representatives/{{ $object->id }}"  class="sweet_warning" href="#"><i class="icon-trash"></i></a></li>

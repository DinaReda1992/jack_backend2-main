...
<!-- 						</div> -->
				...
				<!-- Content area -->
				<div class="content">
					<form method="post" class="form-horizontal" action="/provider-panel/add-report/{{ $order }}" enctype="multipart/form-data">
						{!! csrf_field() !!}
											...
											<div class="col-lg-10">
												@foreach( \App\Models\ReportPhotos::where('report_id',$object->id)->where('type',1)->get() as $photo)
													<div align="center" style="height: 75px;width: 100px;float: right;margin-right: 5px">
														...
														<a href="/provider-panel/delete-photo-report/{{ $photo->id }}" style="text-align: center">حذف</a>
													</div>
												@endforeach
											</div>
										</div>

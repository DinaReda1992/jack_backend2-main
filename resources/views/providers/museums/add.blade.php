				...
				<div class="page-header">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">لوحة التحكم </span> - {{ isset($object)? 'تعديل':'إضافة' }} المعرض</h4>
						</div>
...
<!-- 						</div> -->
					...
					<div class="breadcrumb-line">
						<ul class="breadcrumb">
							...
							<li><a href="/provider-panel/museums">عرض المعارض</a></li>
							<li class="active">{{ isset($object)? 'تعديل':'إضافة' }} المعرض</li>
						</ul>
...
@include('admin.message')
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">{{ isset($object)? 'تعديل':'إضافة' }} المعرض </h5>
							<div class="heading-elements">
								<ul class="icons-list">
						...
						<div class="panel-body">
							...
							<form enctype="multipart/form-data"  method="post" class="form-horizontal" action="{{ isset($object)? '/provider-panel/museums/'.$object->id : '/provider-panel/museums'  }}">
								{!! csrf_field() !!}
									@if(isset($object))
                    	...
                    	@endif
									...
									<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
										<label class="control-label col-lg-2">أدخل اسم المعرض</label>
										<div class="col-lg-10">
										<input type="text" name="name" value="{{ isset($object) ? $object->name  : old('name')  }}" class="form-control" placeholder="أدخل اسم المعرض">
										@if ($errors->has('name'))
									  ...
									  <div class="form-group{{ $errors->has('user_id') ? ' has-error' : '' }}">
										<label class="control-label col-lg-2">إختر العضو صاحب المعرض</label>
										<div class="col-lg-10">
													...
													<select name="user_id" data-placeholder="ابحث بالاسم" class="select-minimum">
														<option></option>
														<optgroup label="إختر العضو صاحب المعرض">
										...
										<option value="">إختر العضو صاحب المعرض</option>
										@foreach($users as $user)
										<option value="{{ $user -> id }}"  {{ isset($object) && $object->user_id==$user->id ? 'selected' : (old('user_id') == $user->id ? 'selected' : '') }}>{{ $user->full_name() . " ( " . $user->email . "  )" }}</option>
										@endforeach
										</select> --}}
									...
									<div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
										<label class="control-label col-lg-2">نبذة عن المعرض</label>
										<div class="col-lg-10">
									...
									<label class="control-label col-lg-2">أدخل صورة المعرض</label>
											<div class="col-lg-10">
								...
								<div class="text-right">
									<button type="submit" class="btn btn-primary">{{ isset($object)? 'تعديل':'إضافة' }} المعرض <i class="icon-arrow-left13 position-right"></i></button>
								</div>
							</form>
						</div>
					</div>

        ...
        <section class="invoice">
            ...
            <div class="row invoice-info">
                ...
                <div class="col-sm-4 invoice-col">
                    ...
                    <span>
                        {{ __('dashboard.order_status') }}:<strong>{{ @$object->orderStatus->name }}</strong></span><br>
                    @if ($object->parent_order)
                        <span>{{ __('dashboard.recharge_of_an_missing_order_the_number') }}
                            {{ $object->parent_order }}</span><br>
                    @endif
                    @if ($object->payment_method == 0 && $object->parent_order == null)
                        <span> {{ __('dashboard.order_status') }}: {{ __('dashboard.under_review') }}</span><br>
                        <span>
                    ...
                    @elseif($object->parent_order)
                        <span> {{ __('dashboard.payment_status') }}:
                            {{ __('dashboard.paid_due_to_missing_on_order_no') }}{{ $object->parent_order }}</span><br>
                    @else
                        <span> {{ __('dashboard.payment_type') }}:{{ @$object->paymentMethod->name }}</span><br>
                    @endif
                    @if ($object->scheduling_date != null)
                        <span> {{ __('dashboard.order_type') }}:{{ __('dashboard.scheduled_orders') }}</span><br>

        ...
        <div class="page-header">
            <div class="page-header-content">
                <div class="page-title">
                    <h4><i class="icon-arrow-right6 position-left"></i> <span
                        ...
                        {{ isset($object) ? __('dashboard.edit') : __('dashboard.add') }} {{ __('dashboard.category') }}</h4>
                </div>
                <div class="heading-elements">
                    <div class="heading-btn-group">
                        <a href="/admin-panel/categories"><button type="button" class="btn btn-success" name="button">
                                {{ __('dashboard.view_categories') }} </button></a>
                    </div>
                </div>
            </div>
            ...
            <div class="breadcrumb-line">
                <ul class="breadcrumb">
                    ...
                    <li><a href="/admin-panel/categories">{{ __('dashboard.view_categories') }} </a></li>
                    <li class="active">{{ isset($object) ? __('dashboard.edit') : __('dashboard.add') }}
                        {{ __('dashboard.category') }}</li>
                </ul>
            </div>
        </div>
        <!-- /page header -->
            ...
            <div class="panel panel-flat">
                ...
                <div class="panel-body">
                    ...
                    <form enctype="multipart/form-data" method="post" class="form-horizontal"
                        action="{{ isset($object) ?  '/' . app()->getLocale() .'/admin-panel/categories/' . $object->id :  '/' . app()->getLocale() .'/admin-panel/categories' }}">
                        {!! csrf_field() !!}
                        @if (isset($object))
                        ...
                        <fieldset class="content-group">
                            ...
                            <div class="form-group{{ $errors->has('parent_id') ? ' has-error' : '' }}">
                                <label class="control-label col-lg-2">{{ __('dashboard.main_category') }}</label>
                                <div class="col-lg-10">
                                    <select name="parent_id" class="form-control">
                                        <option value="">{{ __('dashboard.main_category') }}</option>
                                        @foreach (\App\Models\MainCategories::all() as $category)
                                            <option value="{{ $category->id }}"
                                                {{ isset($object) && $object->parent_id == $category->id ? 'selected' : (old('parent_id') == $category->id ? 'selected' : '') }}>
                            ...
                            <div class="form-group{{ $errors->has('measurementUnits') ? ' has-error' : '' }}">
                                <label class="control-label col-lg-2">{{__('dashboard.select_units_of_measurement')}}</label>
                                <div class="col-lg-10">
                                    <select class="select-multiple-tokenization" multiple="multiple"
                                        name="measurementUnits[]" id="categories">
                        ...
                        <div class="text-right">
                            <button type="submit"
                                ...
                                {{ __('dashboard.category') }} <i class="icon-arrow-left13 position-right"></i></button>
                        </div>
                    </form>
                </div>
            </div>

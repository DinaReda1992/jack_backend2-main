				...
				<div class="page-header">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">لوحة التحكم</span> -    عرض الحسابات التي لم يتم تحصيلها </h4>
						</div>
...
<!-- 						</div> -->
					...
					<div class="breadcrumb-line">
						<ul class="breadcrumb">
							...
							<li class="active"><a href="">عرض الحسابات التي لم يتم تحصيلها  </a></li>
						</ul>
...
@include('admin.message')
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">عرض الحسابات التي لم يتم تحصيلها  </h5>
							<div class="heading-elements">
								<ul class="icons-list">
						...
						<table class="table datatable-colvis-basic">
							...
							<tr>
								...
								<th>مجموع أرباح التطبيق اللي لم يتم تحصيلها</th>
								<th>تحصيل الارباح</th>
								<th>عرض التفاصيل</th>
							</tr>
							</thead>
							...
							@foreach($objects as $object)
								<tr parent_id="{{ $object->id }}" style="{{ $object->getUnpaidRepresentativeDetails($object->representative_id) >= \App\Models\Settings::find(12)->value  ? "background-color: #ffe2f3" : " " }}" >
									<td>{{ $object->id }}</td>
									<td>{{ $object->getRepresentative->username }}</td>
									<td>
										@php $i=0 @endphp
										@foreach(\App\Models\Orders::where('representative_id',$object->representative_id)->where('status',2)->where('payment',0)->get() as $order)
											<?php $i+= $order->price_after_discount ? $order->price_after_discount : $order->final_price  ?>
										@endforeach
										{{ $i  }} ريال
									</td>
{{--									<td>{{ $object->getUser->username }}</td>--}}
									<td>{{ $object->getUnpaidRepresentativeDetails($object->representative_id) }} ريال </td>
									<td>
										<a href="/admin-panel/approve-payment/{{ $object->representative_id }}">
											<button type="button" name="button" class="btn btn-success"> تم تحصيل المبلغ</button>
										</a>
									</td>
									<td><a href="/admin-panel/get-details/{{ $object->representative_id }}"> عرض التفاصيل</a></td>

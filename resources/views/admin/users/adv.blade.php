		...
		<div class="page-header">
			<div class="page-header-content">
				<div class="page-title">
					<h4><i class="icon-arrow-right6 position-left"></i> <span class="text-semibold">{{__('dashboard.dashboard')}}</span> - عرض المتاجر {{app('request')->input('type')=='deleted'?' المحذوفة':''}}</h4>
				</div>
			...
			<div class="breadcrumb-line">
				<ul class="breadcrumb">
					...
					<li class="active"><a href="">عرض العضويات المميزة</a></li>
				</ul>
		...
		<!-- Basic example -->
			<div class="panel panel-flat">
				<div class="panel-heading">
					<h5 class="panel-title">عرض العضويات المميزة</h5>
					<div class="heading-elements">
						<ul class="icons-list">
							<li>
								@if(app('request')->input('type')=='deleted')
									<a style="color: blue" href="/admin-panel/provider-users" > {{__('dashboard.active')}} <i class="fa fa-gift"></i></a>

								@else
									<a style="color: red" href="/admin-panel/provider-users?type=deleted"> المحذوف <i class="icon-trash"></i></a>
								@endif
							</li>
...
{{--						<th>الغاء الباقة</th>--}}
					...
					@foreach($objects as $object)
						<tr parent_id="{{ $object->id }}">
							<td>{{ $object->id }}</td>
							<td>
								<a  href="/admin-panel/balance-details/{{$object->id}}">
								{{ $object->username }}
								</a>
							</td>
								...
								<ul class="icons-list">
									@if($object->is_archived==1)
										<li class="text-teal-600"><a onclick="return false;"
																	 object_id="{{ $object->id }}"
																	 method="get"
																	 delete_url="/admin-panel/provider_archived_restore/{{ $object->id }}"
																	 class="sweet_warning" method="get" href="#" message="هل انت متأكد من استعادة المتجر"><i

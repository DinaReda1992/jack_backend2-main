...
$status='تم الغاء الطلب';
    $color='red';
						...
						<tr parent_id="{{ $object->id }}">
							...
							<td>
								<a href="/admin-panel/balance-details/{{$object->user_id}}">

								<img class="shop-img" src="/uploads/{{@$object->getUser->photo?@$object->getUser->photo:'placeholder.png' }}">{{ @$object->getUser->username }}
								</a>
							</td>
							<td>
								<a href="/admin-panel/balance-details/{{$object->user_id}}">
								{{ \App\Models\Balance::where('user_id',$object->user_id)->sum('price')-\App\Models\Balance::where('user_id',$object->user_id)->sum('site_profits') }}
								</a>
							</td>

							<td>{{ $object->price }}</td>
							<td>
							<a target="_blank" href="/admin-panel/bank_accounts?user_id={{$object->user_id}}">
								عرض الحسابات
							</a>
							</td>
							...
							<td align="center" class="center">
								@if($object->status==0)
									<a class="btn btn-success" href="/admin-panel/withdraw-order/{{$object->id}}">
										 تحويل
									</a>


									<a onclick="return false;" data-toggle="modal"
									   data-target="#cancelForm{{ $object->id }}555" href="#">
										<button type="button" name="button" class="btn btn-danger"> رفض</button>
									</a>
									<div class="modal fade" id="cancelForm{{ $object->id }}555" tabindex="-1" role="dialog"
										...
										<div class="modal-dialog" role="document">
											<div class="modal-content">
												...
												<form method="get" action="/admin-panel/cancel_request_money/{{ $object ->id }}">

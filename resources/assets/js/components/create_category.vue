<template>
  <div>
    ...
    <div class="form-group position-relative">
      ...
      <input type="checkbox" @change="changeIfOffer" />
    </div>
    <div class="form-group position-relative" v-if="!is_offer">
      <label>{{ $t('main.Search In Categories') }}</label>
      <select class="form-control" v-model="category_id" @change="changeCategory" :placeholder="$t('main.Search In Categories')">
              ...
              <div class="modal-body">
                ...
                <div class="_2d71edc9" id="infinite_list">
                  <div v-for="(product, index) in products" class="eb7d03bb">
                    ...
                    <p class="b0c57c6a">{{ $t('main.SAR') }} {{ product.original_price }}</p>
                  </div>
      ...
      <table class="table table-bordered">
        ...
        <tbody>
          <tr v-for="(product, index) in all_products">
            ...
            <td>
              <img :src="'/uploads/' + product.photo" width="40" height="40" />
              {{ product.title }}
            </td>
          </tr>
        </tbody>
    ...
    <div class="text-center">
      <button class="btn btn-success" :disabled="!is_offer && all_products.length == 0 && !category_id"
  ...
  methods: {
    ...
    confirm_order: async function () {
      try {
        ...
        let res = await axios.post('/' + this.lang + "/admin-panel/page-categories", formData);
        if (res.status == 200) {
          // this.loading=false

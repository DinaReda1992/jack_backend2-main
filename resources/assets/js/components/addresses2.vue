<template>
  <div class="panel-body">
    <div class="row ">
      ...
      <div class="col-md-6">
        <div class="">
          <!-- <label class="d-block">اضافة عنوان</label> -->
          <form @submit.prevent="store_address" id="addAddress" class="col-md-8 border p-4">
            ...
            <div class="form-group mb-2">
              <label>بحث فى الخريطة</label>
              <div class="d-flex justify-content-between align-items-center p-3 form-control">
                <gmap-autocomplete placeholder="بحث فى الخريطة" @place_changed="setPlace">
                </gmap-autocomplete>
                <div><a href="#" @click.prevent="usePlace"><i class="icon iconpicker-search"></i></a></div>
              </div>
            </div>
            ...
            <div class="border d-flex align-items-center position-relative mt-2 p-3">
              <div class="position-absolute toggle-drop">
                ...
                <ul class="dropdown-menu">
                  <li><a class="dropdown-item" href="#" @click.prevent="edit_address(item)">{{ $t('main.Edit') }}</a></li>
                  <li><a class="dropdown-item" href="#" @click.prevent="delete_address(item.id)">{{ $t('main.Delete') }}</a>
                  </li>
                </ul>
              </div>
  ...
  methods: {
    ...
    store_address: async function () {
      try {
        ...
        let res = await axios.post('/' + this.lang + '/admin-panel/all-users/addresses/store', formData);
        if (res.status == 200) {
          this.loading = false
    ...
    update_address: async function () {
      try {
        ...
        let res = await axios.post('/' + this.lang + '/admin-panel/all-users/addresses/update/' + this.updated_address.id, formData);
        if (res.status == 200) {
    ...
    delete_address: async function (address) {
      try {
        ...
        var r = confirm(this.$t('main.Are you sure you want to delete this address?'));
        ...
        if (r == true) {
          ...
          let res = await axios.post('/' + this.lang + '/admin-panel/all-users/addresses/delete', params);
          if (res.status == 200) {
            this.loading = false
            this.all_addresses = res.data.addresses
            this.$toast.success({
    ...
    is_home: async function (id) {
      try {
        ...
        var r = confirm("هل أنت متأكد من جعل العنوان هو الرئيسي؟")
        if (r == true) {
          // this.$toast.removeAll()
          this.loading = true
          let params = {
          ...
          let res = await axios.post('/' + this.lang + '/admin-panel/all-users/addresses/home', params);
          if (res.status == 200) {
            this.all_addresses = res.data.addresses
            this.$toast.success(res.data.message)
          } else {

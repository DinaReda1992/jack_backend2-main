...
use App\Http\Resources\ProductsGiftsResource;
use Illuminate\Pagination\LengthAwarePaginator as Paginator;

class CartOrderRepository extends Controller
{
    public static function addToCart($request, $user)
    {
        ...
        if ($product->cart_count) {
            ...
            //     $mount = $cart_item->quantity + 1;
            //     if ($request->recalcItem) {
            //         $mount = $cart_item->quantity;
            //     }
            //     if ($mount > $product->quantity || ($product->quantity - ($mount - 1)) <= $product->min_quantity) {
            //         return response()->json([
            //             'status' => 400,
            //             'message' => __('trans.There is no stock available for this product.')
            //         ], 201);
            ...
            //                 "quantity" => 0,
            //                 "get_quantity" => 0,
            //                 "message" => __('trans.Removed from cart'),
            //             ]
            //         );
            //     }
        ...
        if ($offer) {
            if ($offer->number_of_users || $offer->one_user_use) {
                $allUsesCount = CartItem::where('offer_id', $offer->offer_id)
                    ->where('shipment_id', 0)
                ...
                if ($allUsesCount && ($allUsesCount->allOfferUses >= $offer->number_of_users || $allUsesCount->userUses >= $offer->one_user_use)) {
                    ...
                    $cart_item->offer_id = 0;
                    // if ($cart_item->cart_gifts) {
                    //     CartItem::where('cart_id', $cart_item->id)->delete();
                    // }

                    if ($offer->type_id === 4) {
                        $cartGiftsCount = CartItem::where('user_id', $user->id)
                            ->where('order_id', 0)
                            ->where('shipment_id', 0)
                            ->get();
                        $offerItems = Shop_offer_item::where('offer_id', $offer->offer_id)
                            ->where('group', 1)
                            ->pluck('shop_product_id')->toArray();

                        $cartGiftsQuantity = CartItem::where('user_id', $user->id)
                            ->where('order_id', 0)
                            ->where('shipment_id', 0)
                            ->whereIn('item_id', $offerItems)
                            ->sum('quantity');

                        if (
                            $mount >= $offer->quantity ||
                            (count(array_intersect(collect($cartGiftsCount)->pluck('item_id')->toArray(), $offerItems)) > 0
                                && $cartGiftsQuantity >= $offer->quantity)
                        ) {
                            $lang = App::getLocale();
                            ...
                            $gifts = $objects = Shop_product::select(
                                ...
                                ->where('shop_offer_items.offer_id', $offer->offer_id)
                                ->groupBy('shop_products.product_id')
                                ->get();
                            $gifts = ProductsGiftsResource::collection($gifts);
                            $cart_item->offer_id = $offer->offer_id;
                            $perc = $offer->is_free ? ' 100% ' : $offer->percentage . '%';
                            $gift_message = __('trans.Get a discount on the following products by ') .
                                $perc .
                                __('trans.by adding') . $offer->get_quantity . __('trans.from_it');
                        }
                    }

                    $items = CartItem::where('type', 1)->where(['status' => 0, 'order_id' => 0])->whereHas('product')->with('product')->where('user_id', $user->id)->get();
                    return response()->json(
                        [
                            ...
                            'cart_count' => (int)$user->cartCount(),
                            // 'gifts' => $gifts ?: [],
                            // 'had_gifts' => $cart_item->cart_gifts->count() ? 1 : 0,
            ...
            //     $cobon = MotivationCobon::where('code', $request->code)
            //         ->whereHas('motivation', function (Builder $query) use ($shop_product) {
            //             $query->where('status', 1)->where('user_id', $shop_product->user_id);
            //         })
            //         ->with('motivation')
            //         ->where('shop_product_id', $request->product_id)->first();
            //     if ($cobon) {
            ...
            //             $cart_item->motivation_id = $cobon->motivation_id;
            //             $cart_item->motivation_user_id = $cobon->provider_id;
            //             $cart_item->motivation_cobon_id = $cobon->id;
            //             $available_codes = $cobon->usage - $mount;
            //             if ($available_codes >= $mount) {
            //                 $cart_item->motivation_quantity = $mount;
            //                 $cart_item->motivation_discount = $cobon_value * $mount;
            //                 $cart_item->motivation_vat = $cart_item->motivation_discount * ($tax / 100);
            //             } else {
            //                 $cart_item->motivation_quantity = $available_codes;
            //                 $cart_item->motivation_discount = $cobon_value * $available_codes;
            //                 $cart_item->motivation_vat = $cart_item->motivation_discount * ($tax / 100);
            //             }
            //         } else {
            //             $cart_item->motivation_id = 0;
            //             $cart_item->motivation_user_id = 0;
            //             $cart_item->motivation_cobon_id = 0;
            //             $cart_item->motivation_discount = 0;
            //             $cart_item->motivation_quantity = 0;
            //         }
            //     }
            // }
            // Log::info($offer->type_id);
            switch ($offer->type_id) {
                ...
                case 3:
                    ...
                    if ($mount == $all_quantity) {
                        ...
                        $discount_price = $offer->get_quantity * $discount;
                ...
                case 4:
                    ...
                    $cartGifts = CartItem::where('user_id', $user->id)
                        ->where('order_id', 0)
                        ->where('shipment_id', 0)
                        ->where('cart_id', '!=', 0)
                        ->count();
                    ...
                    if ((count(array_intersect(collect($cartGiftsCount)->pluck('item_id')->toArray(), $offerItems)) > 0
                        && $cartGiftsQuantity >= $offer->quantity && $cartGifts == 0)) {
                        $lang = App::getLocale();
                        $select_product_title = $lang == 'ar' ? 'products.title' : 'products.title_en as title';
                        $gifts = $objects = Shop_product::select(
                            'shop_products.id',
            ...
            //                                 $cart_item->motivation_user_id = $cobon->provider_id;
            //                                 $cart_item->motivation_cobon_id = $cobon->id;
            //                                 $available_codes=$cobon->usage-$cobon_value;
            //                                 if($mount>=$available_codes){
            //                                     $cart_item->motivation_quantity = $mount;
            //                                     $cart_item->motivation_discount = $cobon_value * $mount;
            //                                 }else{
            //                                     $cart_item->motivation_quantity = $available_codes;
    ...
    public static function removeFromCart($request, $user)
    {
        $cart_item = CartItem::where('item_id', $request->id)
            ->where('shipment_id', 0)->where('user_id', $user->id)->first();
        if (!$cart_item) {
            return response()->json(
                [
                    'status' => 400,
                    'cart_count' => (int)$user->cartCount(),
                    'message' => __('trans.This product is not in the cart'),
                ]
            );
        }
        CartItem::where('cart_id', $cart_item->id)->delete();
        ...
        return response()->json(
            [
                'status' => 200,
                'cart_count' => (int)$user->cartCount(),
                'message' => __('trans.Deleted successfully'),
            ]
        );
    }
        ...
        if ($items && count($items) > 0) {
            foreach ($items as $item) {
                if (!empty($item) && $item->product_id) {
                    ...
                    $gift_product = Shop_product::select('shop_products.id', 'products.client_price', 'products.price', 'products.has_tax')
                        ->join('products', 'products.id', 'shop_products.product_id')
    ...
    public static function getCartItems($user, $platform = 'api')
    {
        ...
        $current_items = CartItem::select(
            ...
            // 'cart_items.motivation_cobon_id',
            'cart_items.cart_id',
            'products.client_price',
        )
            ->selectRaw('(CONCAT ("' . url('/') . '/uploads/", products.photo)) as photo')
            ...
            if ($item->offer_id && (!$offer || !$offerItem)) {
                ...
                switch ($offer->type_id) {
                    ...
                    case 1:
                        ...
                        if ($offer->price_discount <= $price && $cart_discount != $offer_discount) {
                        ...
                        // discount percentage
                    case 2:
                        $offer_discount = round($item->quantity * ($price * $offer->percentage / 100), 2);
                    ...
                    case 3:
                        ...
                        $quantity_offer_in_cart = (int)($item->quantity / ($offer->quantity + $offer->get_quantity));
                        $offer_discount = ($offer->get_quantity) * $discount * $quantity_offer_in_cart;
    ...
    public static function addOrder($request, $user, $platform = 'api')
    {
        ...
        if ($platform == 'website') {
            ...
            return view('website.payment_method', compact('order', 'balance', 'handDeliveryCost', 'banks', 'app_banks'));
        }
    ...
    public static function sendOrder($request, $user, $platform = 'api')
    {
        ...
        switch ($request->payment_type) {
            ...
            case "bank":
                ...
                if ($request->hasFile('photo')) {
                    ...
                    $fileName = self::uploadOne($file, $destinationPath, $name);
                    $transfer->photo = $fileName;
                }
                $transfer->save();
                $sendOrder = (new self)->prepareOrder($request, $objects, $tax, $coupon, $coupon_money, 0, 4, $user, $coupon_percentage);
                break;
            case "balance":
        ...
        if (!$sendOrder) {
            if ($platform == 'website') {
                return redirect('/cart/')->with('error', __('trans.Order not sent'));
...
';
        ...
        if ($platform == 'website' && $request->payment_type == 'payment') {
            return redirect('/thank-you?id=' . $order_id);
    ...
    public static function uploadOne(UploadedFile $file, $folder = null, $filename = null, $disk = 'public')
    {
    ...
    public static function prepareOrder($request, $shipments, $tax, $code, $coupon_money, $handDeliveryCost, $order_type, $user, $coupon_percentage = 0)
    {
        ...
        for ($y = 0; $y < $cart_items->count(); $y++) {
            ...
            if (in_array($cart_items[$y]['id'], $cartItemsIds)) {
                ...
                CartItem::where(['id' => $cart_items[$y]['id']])->update(['cobon_discount' => $cartCouponDiscount, 'status' => 1]);
            }
            //
        ...
        if ($request->with_balance == 1) {
            ...
            if ($user_balance < $total && $user_balance > 0) {
                $balance = new Balance();
                $balance->user_id = $user->id;
                $balance->price = -$user_balance;
    ...
    public static function getSummaryCost($shipments, $tax)
    {
        ...
        //        }
        $shipingVat = $shipmentTotal * $tax / 100;
        $total = $subtotal + $vat +   $shipingVat + $shipmentTotal - $discounts;

    ...
    public function create($token=null)
    {
        ...
        if($token && $order){
            ...
            if(request()->user_id){
                ...
                $carts=$carts=CartItem::where('order_id',0)->whereHas('user')->where('status',0)->where('user_id',request()->user_id)
                    ->get();
...
//        $order->status=1;
        ...
        $order->save();
        CartItem::where('order_id',$order->id)->update(['user_id'=>$order->user_id]);
...
//        echo nl2br($ret);
    ...
    public function store_item(Request $request)
    {
        ...
        foreach (json_decode($request->ids) as $id){
            ...
            $ifExist=CartItem::where('status',0)->where('item_id',$product->id)->where('order_id',$request->order_id)->first();
            if($ifExist){
                continue;
            }else{
                $order=Orders::find($request->order_id);
    ...
    public function update_item(Request $request)
    {
        ...
        $input=$request->all();
        $items=CartItem::where('id',$request->id)->first();
